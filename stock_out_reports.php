...
if (isset($_GET['month'])) {
    ...
    if ($selected_month != '0') {
        $sql = "SELECT * FROM reports WHERE transaction_type = 'Stock-Out' AND MONTH(date) = $selected_month order by date DESC";
    } else {
        $sql = "SELECT * FROM reports WHERE transaction_type = 'Stock-Out' order by date DESC";
    }
} else {
    $sql = "SELECT * FROM reports WHERE transaction_type = 'Stock-Out' order by date DESC";
}
...
<body>
    ...
    <script>
        function filterByMonth() {
            ...
            if (selectedMonth != '0') {
                window.location.href = `stock_out_reports.php?user_id=<?php echo $user_id; ?>&month=${selectedMonth}`;
            } else {
                window.location.href = `stock_out_reports.php?user_id=<?php echo $user_id; ?>`;
            }
        }
    </script>
